;(function () {
	...
	var scrlTop =  function() {
		$('.JBC-gotop').click(function(event){
			$('html, body').animate({
		        scrollTop: 0
		    }, 500, 'easeInOutExpo');
	...
	var imgHover = function() {
		$('.JBC-project > a').mouseenter(function(){
			$(this).find('.JBC-overlay, .JBC-overlay-text').stop().animate({
				opacity: 1
			}, 400 );
		}).mouseleave(function(){
			$(this).find('.JBC-overlay, .JBC-overlay-text').stop().animate({
	...
	var accordion = function() {
		$('.js-JBC-accordion-toggle').click(function(event){
			var $this = $(this);
			if ( $this.closest('.JBC-accordion').find('.JBC-accordion-body').is(':visible') ) {
				$this.text('Show More');
			} else {
				$this.text('Show Less');
			}
			$this.closest('.JBC-accordion').find('.JBC-accordion-body').slideToggle(400, 'easeInOutExpo');
			event.preventDefault();
		})
	};
	...
	var animateSocial = function() {
		if ( $('#JBC-intro-social').length > 0 ) {
			$('#JBC-intro-social > a').each(function( k ) {
	...
	var heroWayPoint = function() {
		if ( $('#JBC-header').length > 0 ) {
			$('#JBC-header').waypoint( function( direction ) {
